...
serve(async (req) => {
  ...
  try {
    ...
    // Balance check
    const { data: credits } = await admin.from("credits_ledger").select("amount").eq("user_id", user.id);
    const balance = (credits || []).reduce((s: number, r: any) => s + Number(r.amount), 0);
    if (balance < INSTALL_COST) return new Response(JSON.stringify({ error: `Créditos insuficientes. Precisa de ${INSTALL_COST}.` }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    ...
    if (edges && edges.length) {
      ...
      if (toInsert.length) await admin.from("agent_flow_edges").insert(toInsert);
    }

    // Debit buyer, credit author, record install, bump counter
    await admin.from("credits_ledger").insert({
      user_id: user.id, amount: -INSTALL_COST, type: "usage",
      description: `Instalação do fluxo "${flow.name}" do Marketplace`,
      reference_id: `flow-install-${flowId}`,
    });
    await admin.from("credits_ledger").insert({
