    ...
    <div className="flex min-h-screen bg-[hsl(220,25%,5%)]">
      <div className="hidden lg:flex lg:w-1/2 items-center justify-center p-12 border-r border-white/10">
        <div className="max-w-md text-center">
          ...
          <div className="mt-8 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-white">
            ...
            <span className="font-medium">{t("signup.bonus")}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full items-center justify-center p-8 lg:w-1/2">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <h1 className="mb-2 font-display text-3xl font-bold text-white">{t("signup.title")}</h1>
          <p className="mb-8 text-white/50">{t("signup.desc")}</p>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="name" className="text-white/70">{t("signup.name")}</Label>
              <Input id="name" placeholder="Seu nome" value={name} onChange={(e) => setName(e.target.value)} required className="border-white/10 bg-white/[0.05] text-white placeholder:text-white/30 focus-visible:ring-white/20" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="email" className="text-white/70">{t("signup.email")}</Label>
            ...
            <div className="space-y-2">
              <Label htmlFor="password" className="text-white/70">{t("signup.password")}</Label>
              <Input id="password" type="password" placeholder="Mínimo 6 caracteres" value={password} onChange={(e) => setPassword(e.target.value)} minLength={6} required className="border-white/10 bg-white/[0.05] text-white placeholder:text-white/30 focus-visible:ring-white/20" />
            </div>
          ...
          <p className="mt-6 text-center text-sm text-white/40">
            {t("signup.hasAccount")}{" "}
            <Link to="/login" className="font-medium text-[hsl(174,62%,47%)] hover:underline">{t("signup.login")}</Link>
          </p>
        </div>
      </div>
    </div>
